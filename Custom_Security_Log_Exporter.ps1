    ...
    foreach ($event in $events) {
        ...
        if ($event.properties[19].Value.Length -ge 5) {
            ...
            if (-Not ($log_contents -match "$($timestamp)") -or ($log_contents.Length -eq 0)) {
                ...
                #
                "latitude=$($latitude) longitude=$($longitude) label=$($country)-$($sourceIp) sourcehost=$($sourceIp) country=$($country)" | Out-File $LOGFILE_PATH -Append -Encoding utf8

                Write-Host -BackgroundColor Black -ForegroundColor Magenta "latitude=$($latitude) longitude=$($longitude) label=$($country)-$($sourceIp) sourcehost=$($sourceIp) country=$($country)"
            }
            else {
